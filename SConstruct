...
# binary assets
track_data = "assets/tracks/*.bin"
t3d_models = "assets/*.glb"
...
lib_tiny3d = ["libraries/tiny3d/build/libt3d.a"]
lib_flecs = ["libraries/flecs/libflecs_static.a"]
...
env.Command(
    ...
    action=[
        Action(
            chdir="libraries/flecs",
            act=f"cmake -DN64_INST={N64_INST} -DCMAKE_TOOLCHAIN_FILE=../n64_flecs.cmake .",
        ),
...
def build_dfs(target, source, env):
    ...
    for t3d_model in env.Glob(t3d_models):
    ...
    for asset in env.Glob(track_data):
        result = env.Execute(
...
def build_n64_rom(target, source, env):
    ...
    result = env.Execute(
        f"N64_INST={N64_INST} {n64_tools['N64SYM']} {elf_file} {sym_file}"
    )
    if result != 0:
        return result
    ...
    # Compress the ELF
    env.Execute(
        f"N64_INST={N64_INST} {n64_tools['N64ELFCOMPRESS']} -o build/ -c 1 {stripped_file}"
    )
...
# build the dfs
dfs = env.N64DFS(target="build/Build.dfs", source=elf)
# Build the ROM
