 ...
 *
 * BezierTrack allows to define a track as a chain of cubic Bezier curve
 * segments. Each segment is defined by 4 control points, where the last point
 * of each segment is the first point of the next segment. The class allows to
 * retrieve interpolated points along the curve, including extra data such as
 * normals and widths associated with each segment.
 *
 ...
 *
 * The last control point of segment D (A0) is a duplicate of the first control
 * point of segment A to allow proper looping.
 *
...
public:
  ...
  inline BezierTrack(uint segments_)
      : control_points(segments_ * 3 + 1), segments_data(segments_ + 1) {}
  ...
  static BezierTrack *from_blender_track_data(size_t numSegments,
